...
## 封印を解いている間はツールバーのアイコンが開眼 ✅
- [x] 開眼アイコン（赤い枠・暗い赤のまぶた・光る赤い瞳、16px はドット打ち）
- [x] バックグラウンドで状態に合わせてアイコンを切り替え（時間切れはアラームで）
- [x] テスト・ビルド・確認（実機の切り替えは昼に確認）

## 封印を解く画面の磨き込みとブロック表示の文字選択対策 ✅
- [x] 分数入力を標準のスピナーからスライダーに（長いほど白 → 金 → 赤 → 赤黒）
- [x] 開眼時に星の点々も目にする（金の閉じた目から同時に開く、ツールバーのアイコンにも）
- [x] ブロック表示の文字を選択しても見えないように
- [x] テスト・ビルド・確認

## 点々の演出パターン再検討と長時間の「やばさ」強化 ✅
- [x] 点々の扱いを複数パターンで比較（モック）→ 黒に赤い点（R1）
- [x] まぶたの開き方を刷新（上まぶたがせり上がる）
- [x] 120分付近の表現を「見づらい」から「本気でやばい」に（壊れる + 部屋ごと）
- [x] 実装・確認
- [x] v1.4.0 リリース
